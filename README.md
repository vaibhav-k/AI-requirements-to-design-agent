...
* **`backend/orchestrator/`** — owns all state and every LLM call.
  ...
  Microsoft Agent Framework-backed requirements/design generation, and
  an external MCP server exposing the whole pipeline to outside clients
  (an IDE assistant, another agent). This is the service end users and
...
* **`backend/tools-service/`** — the deterministic, LLM-free logic that
  ...
  rendering and design validation. Plain FastAPI REST, no MCP awareness,
  no LLM dependency.
* **`backend/mcp-wrapper/`** — a thin internal MCP gateway between the
  orchestrator and tools-service. Translates two MCP tool calls
  (`generate_architecture_diagram_tool`, `validate_system_design_tool`)
  into plain REST calls against tools-service. Nothing outside this
  repository talks to it.

```text
                     ┌─────────────────────┐
  End user / IDE ──► │ backend/orchestrator │  owns state + every LLM call
  assistant / other  │                      │
  MCP client         └──────────┬───────────┘
...
generation and design validation are the two capabilities that need
tools-service/mcp-wrapper up). Each runs as a plain local process — no
...
**Never run mypy, Pyright/Pylance, or a SonarQube/SonarLint scan across
the whole repo root in one shot** (`mypy .` from here, or opening this
folder as a single VS Code workspace with Pylance's default "analyze the
whole open folder" behavior). Each service deliberately reuses generic
top-level package names — `backend/orchestrator` imports as `app`,
`backend/tools-service` and `backend/mcp-wrapper` both import as `src` —
because each is meant to be treated as its own independent Python
project with its own virtualenv, never merged into one. Point a
whole-repo tool at all of them at once and you get exactly the collision
this section exists to head off:

```text
error: Duplicate module named "app" (also at ".\app\__init__.py")
```

(that specific one is from the still-present, superseded root-level
...
because they share that generic name by the same design.) None of this
means the code has real type or lint errors — each service is clean
under mypy and ruff run scoped to just that service's own directory;
it's purely a "which project does this file belong to" ambiguity that
only appears when a tool's search path spans more than one service.
