...
validation concern) stays orchestrator-only. ``DiagramMetadata`` is also
reproduced here even though it's rendering-only (not part of the LLM-
generated artifact) since ``diagram.py`` needs its shape directly.
"""
