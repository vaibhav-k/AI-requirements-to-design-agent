...
**Milestone 2 is complete; Milestone 3 (Design Refinement) is in progress.**

Milestone 2's full requirements→architecture pipeline shipped, and three of
Milestone 3's six planned items are also already done - architecture
refinement, architecture version comparison, and the human approval
workflow. What's left to close out Milestone 3: requirement-to-architecture
...
"Roadmap" → "Milestone 3 - Design Refinement" for the checklist and "Path to
Milestone 3 Completion" (just below it) for concrete next steps on each
remaining item.

Separately from the feature roadmap, a **Clean Architecture
...
architecture/tech initiative orthogonal to the roadmap above - it
didn't add or change user-facing capability, and didn't block Milestone 3
work landing in parallel. See "Clean Architecture Migration" under
"Next Steps" for the full plan and what each slice did.

Also orthogonal to the roadmap: a **fourth session stage, Task
Planning (the Work Breakdown Agent), is now fully wired end to end.**
Given an approved `RequirementsArtifact` + `SystemDesignArtifact`, it
generates a traceable Feature → Story → Task implementation plan
(effort-estimated per task, XS–XL) and exports it as an RFC-compliant,
Monday.com/Azure DevOps-importable CSV. It's reachable three ways now:
the external MCP server's `generate_work_breakdown`/
`refine_work_breakdown`/`export_work_breakdown_csv` tools (unchanged),
the web API's `POST/GET /requirements-runs/{id}/work-breakdown[/refine|
/versions|/{version}|/export]` routes (new - gated on the session's
architecture being approved, with the same "generating"
double-submission guard and version-conflict handling requirements/
architecture already use), and the frontend's new "Task Planning" tab
(new - appears once architecture is approved, lets you generate, refine
conversationally, and download the CSV). CLI support is still open -
...
* Image input classification - an uploaded PNG/JPG/JPEG is automatically
  ...
  stage); see "Next Steps" → "Image Input Classification" below. A
  diagram-originated session starts with zero functional/non-functional
  requirements (deliberately - nothing was actually specified); manual
  requirements editing (`PUT .../requirements`, see "Web API &
  Authentication" below) is how one picks up real requirements
  afterward, e.g. to unblock Task Planning.
...
* Work Breakdown Agent (Task Planning) - a fourth Microsoft Agent
  Framework-backed session stage that turns an approved requirements +
  architecture pair into a traceable Feature → Story → Task plan (every
  Task carries an XS–XL effort estimate and the requirement/architecture
  IDs that justify it), plus a deterministic, LLM-free CSV exporter
  (`backend/tools-service`) that renders the plan and flags any
  fabricated ID, uncovered requirement, or uncovered architecture
  element instead of silently accepting it. Reachable via the external
  MCP server's `generate_work_breakdown`/`refine_work_breakdown`/
  `export_work_breakdown_csv` tools, via the web API's
  `/requirements-runs/{id}/work-breakdown` routes, and via the
...
Detailed database schemas, detailed API specifications, deployment topology, networking, Kubernetes configuration, and implementation-specific infrastructure remain outside Milestone 2 scope.
...
* `app/api/routes/requirements.py` - the requirements → architecture flow
  ...
  * `PUT /requirements-runs/{id}/requirements` - directly set/edit
    structured requirements fields by hand, with **no AI call**. Exists to
    unblock a session that started from an uploaded diagram image: that
    flow deliberately leaves `functional_requirements`/
    `non_functional_requirements` empty rather than invent requirements the
    diagram didn't specify (see "Image Input Classification" below), but
    Task Planning refuses to generate a work breakdown without at least one
    of either - leaving such a session permanently stuck without this
    route. Unlike `refine`, it works from *any* stage except the transient
    `"generating"` one, and never changes `stage` itself. Every field is
    optional and independently means "leave unchanged" when omitted
    (`None`); an explicit empty list clears it. Providing
    `functional_requirements`/`non_functional_requirements` replaces the
    whole list and assigns fresh sequential IDs (`FR-001`, `FR-002`, .../
    `NFR-001`, `NFR-002`, ...) - previous IDs are **not** preserved, which
    is low-risk for the primary (diagram-originated) use case since there
    were no requirement IDs for an architecture to have traced to yet, but
    worth knowing if you use this on a normal text-derived session whose
    architecture already exists. See `edit_requirements` in
    `app/api/routes/requirements.py` for the full contract. An AI-inferred
    suggestion of requirements straight from the diagram is a possible
    future enhancement - explicitly out of scope for this route.
...
* `app/api/routes/work_breakdown.py` - the fourth session stage, Task
  Planning, as HTTP endpoints, following the exact `requirements.py`/
  `artifacts.py` conventions above (`load_owned`, the `"generating"`
  double-submission guard, `_upsert_guarded`'s `SessionConflictError`→409
  translation, error→HTTP mapping):
  * `POST /requirements-runs/{id}/work-breakdown` - generate a breakdown
    from the session's approved requirements + architecture
    (`GenerateSessionWorkBreakdownUseCase`, which wraps the standalone
    `GenerateWorkBreakdownUseCase` from "Work Breakdown Agent" below).
    `409` unless the session is in the `"architecture"` stage with
    `approval_status == "approved"`; `409` if a breakdown already exists
    for this session (use `/refine` instead); `422` on
    `WorkBreakdownGenerationError`/`ValueError` (no requirements, no
    components), reverting the session back to `"architecture"`.
  * `POST /requirements-runs/{id}/work-breakdown/refine` - body
    `{"input": "..."}`; the work-breakdown analogue of
    `refine-architecture`: bumps `work_breakdown_version` and
    re-generates with the previous breakdown as context. `409` unless
    the session is already in the `"work_breakdown"` stage.
  * `GET /requirements-runs/{id}/work-breakdown` - the current
    `WorkBreakdownArtifact`. `404` if none has been generated yet.
  * `GET /requirements-runs/{id}/work-breakdown/versions` /
    `GET /requirements-runs/{id}/work-breakdown/{version}` - the same
    version-history pattern `artifacts.py` uses for requirements/design,
    backed by the same immutable, `overwrite=False` Blob layout
    (`{env}/{session_id}/work-breakdown/v{n}.json`).
  * `GET /requirements-runs/{id}/work-breakdown/export` - calls
    `ExportSessionWorkBreakdownUseCase` (the web-API-facing wrapper
    around `WorkBreakdownExporterPort`/`McpToolsClient.export`, the same
    port the external MCP server's `export_work_breakdown_csv` tool
    uses) and returns the rendered CSV as `text/csv`, caching it in Blob
    Storage (`.../work-breakdown/v{n}.csv`) alongside the JSON artifact.
    `409` if no breakdown exists yet; `422` on `WorkBreakdownExportError`
...
* **Conversation** (`Workspace.tsx` + `Conversation.tsx`) - the AI
  ...
  **Error**. Once a session reaches the `"architecture"` stage: further
  ...
  stuck on `generating`" limitation above, which this doesn't solve. Once
  a session's architecture is **approved**, a "Generate work breakdown"
  action appears alongside the existing ones the same way "Accept &
  generate architecture" does - and once a breakdown exists, further chat
  input for a session in the `"work_breakdown"` stage is routed to
...
* **Artifacts** (`ArtifactPanel.tsx`) - tabs for Requirements,
  ...
  * `RequirementsView.tsx` - summary, business goal, actors,
    ...
    selected version.
  * `RequirementsEditor.tsx` - a manual, non-AI edit form (business
    goal/summary text fields, and add/remove lists for functional/
    non-functional requirements) sitting above `RequirementsView.tsx` on
    the Requirements tab, calling `PUT .../requirements` (see "Web API &
    Authentication" above). Shown open by default, with an explanatory
    prompt, whenever the session currently has zero functional +
    non-functional requirements - the diagram-upload-stuck case this
    route exists for - and collapsed behind an "Edit requirements" toggle
    otherwise. Works at any session stage, unlike chat-based Refine.
  ...
  * `WorkBreakdownView.tsx` - a pure display component (no API calls of
    its own, same as `RequirementsView.tsx`/`ArchitectureView.tsx`)
    rendering the Feature → Story → Task hierarchy: each task shows its
    description, an effort badge (`XS`–`XL`), and its
    requirement/architecture ID traceability tags, plus a distinct
    ambiguities section when the artifact has any. The tab itself stays
    disabled until the session's architecture is approved, and shows an
    empty state pointing at the "Generate work breakdown" action (in
    Conversation, not here - see above) until one exists. An
  ...
  * When a compare version is selected on the Requirements or
    ...
    added/removed/changed entries are tagged and shown before/after, for
    every list field on both artifacts.
...
* ~~Set up GitHub Actions CI~~ - `.github/workflows/ci.yml` now runs
  ...
  earlier revision of this README (and the Milestone 2 checklist) had claimed
...
The pre-existing Milestone 3/"Future" roadmap further down (coverage analysis,
...
As of the tools-service split, this repository is three independently
deployable services rather than one monolith:

```text
backend/orchestrator/    (this directory) - owns all state and every LLM
                          call. FastAPI web API, the CLI (app/main.py),
                          Azure Blob/Cosmos persistence, Microsoft Agent
                          Framework-backed requirements/design/work-
                          breakdown generation, and the pre-existing
                          external MCP server (app/mcp/server.py) all
                          live here.
...
backend/tools-service/    The deterministic, LLM-free logic that used to
                          run in-process on the orchestrator: Graphviz
                          diagram rendering (formerly
                          app.design.diagram.ArchitectureDiagramGenerator)
                          and design validation (formerly
...
**Why keep the external MCP server (`app/mcp/server.py`) separate from
this internal gateway?** They serve different audiences. The external
server exposes the *entire* requirements-to-architecture pipeline
(`analyze_requirements`, `generate_system_design`, etc.) as MCP tools for
outside clients - an IDE assistant, another agent - the same way it did
before this split. The internal gateway (`backend/mcp-wrapper`) exists
purely so the orchestrator can reach deterministic, LLM-free logic that
now lives in a different process; nothing outside this repository ever
talks to it directly. `app/mcp/server.py`'s own
`generate_architecture_diagram`/`validate_system_design` tools still
exist and now call through `McpToolsClient` themselves (see
`_diagram_generator`/`_validator` in that module) rather than the deleted
concrete classes - an external MCP client sees no change in behavior,
only an extra network hop it never has to know about.

**Local development** needs all three services running as plain local
processes (no Docker): tools-service (`uvicorn main:app --port 8100` in
`backend/tools-service`), mcp-wrapper (`python combined_main.py` in
`backend/mcp-wrapper`, gateway on `:8200`), and this orchestrator
(`DESIGN_TOOLS_MCP_URL` in `.env` pointed at
`http://localhost:8200/mcp/design-tools`, the default if unset). See the
root `README.md` (one level up from this directory) for the full
startup order.
...
now spanning the three services above rather than running entirely
in-process; see "Service Architecture" just above for how the last two
boxes below (Diagram Generator, Architecture Validator) are reached:
...
adapter classes directly, and the Diagram Generator and Architecture
Validator boxes now sit behind `DiagramRendererPort` and
`ArchitectureValidatorPort` respectively, rather than
`app.design.session.ArchitectureSession` and its callers (`app/main.py`,
`app/api/dependencies.py`, `app/mcp/server.py`) depending on
`ArchitectureDiagramGenerator`/`ArchitectureValidator`/Graphviz directly.

A later, separate change - the tools-service split described in
"Service Architecture" above - moved what's *behind* both of those ports
out of this process entirely, into `backend/tools-service`, reached over
`backend/mcp-wrapper`'s internal MCP gateway. Both ports' concrete
implementation is now `app.infrastructure.tools_client.McpToolsClient`.

---

## Project Structure

This tree is `backend/orchestrator/`'s own layout - one of the three
services described in "Service Architecture" above. See
`backend/tools-service/README.md` and `backend/mcp-wrapper/README.md`
for those services' structure; the root `README.md` (one level up from
here) has the whole-repo picture.
...
│   │       ├── generate_work_breakdown.py     # standalone: requirements+design -> artifact
│   │       ├── generate_session_work_breakdown.py  # session-aware wrapper used by
...
│   │   ├── comparison.py
│   │   └── session.py
│   │       # validator.py, diagram.py, icons.py, and icons/azure/*.png all
│   │       # moved to backend/tools-service as part of the tools-service
│   │       # split (see "Service Architecture" above) - this directory no
│   │       # longer has an in-process DiagramRendererPort/
│   │       # ArchitectureValidatorPort implementation; both now come from
│   │       # app/infrastructure/tools_client.py instead.
│   │
│   ├── mcp/
│   │   ├── __init__.py
│   │   └── server.py
...
prior revision of this README (and the Milestone 2 checklist below) had claimed
...
The Milestone 2 implementation was completed in six incremental steps.
...
This prevents a partially generated architecture from being presented as a successful Milestone 2 artifact.
...
Milestone 1 converts natural-language requirements into a structured requirements artifact.
...
Milestone 2 consumes an accepted requirements artifact and generates a high-level system architecture.
...
## 4. Install Graphviz (backend/tools-service only)

As of the tools-service split (see "Service Architecture" above),
diagram rendering - and the `graphviz` Python package/`dot` executable it
needs - no longer lives in this directory at all; it moved to
`backend/tools-service`. This step is a no-op for `backend/orchestrator`
itself; skip straight to "5. Configure environment variables" below
unless you're also setting up `backend/tools-service`, whose own README
covers this same step for that service.
...
# Diagram rendering + design validation now run in a separate service -
# see "Service Architecture" above. Defaults to the standard local-dev
# address if unset.
DESIGN_TOOLS_MCP_URL=http://localhost:8200/mcp/design-tools
```
...
Selecting `Accept` triggers architecture generation:
...
* `generate_work_breakdown(requirements_json, design_json)` - the Task
  Planning entry point: an accepted requirements + architecture pair in,
  a structured `WorkBreakdownArtifact` JSON out (Feature → Story → Task,
  each task effort-estimated XS–XL and tagged with the requirement/
  architecture IDs it traces back to). Same underlying
  `GenerateWorkBreakdownUseCase` the web API's
  `POST /requirements-runs/{id}/work-breakdown` route calls.
* `refine_work_breakdown(user_input, requirements_json, design_json, breakdown_json)`
  - re-generates with the previous breakdown as context, same as the web
  API's `.../work-breakdown/refine` endpoint.
* `export_work_breakdown_csv(breakdown_json, requirements_json, design_json)`
  - renders the breakdown as an RFC 4180 CSV via `backend/tools-service`
  (over `backend/mcp-wrapper`'s `export_work_breakdown_tool`), the same
  path the web API's `.../work-breakdown/export` route uses; also
...
## Web API (post-Milestone 2, pre-Milestone 3)
...
* [x] Frontend / UI - chat-first workspace (Conversation + Requirements +
      Architecture + Task Planning), version switching, side-by-side
      version compare, and an interactive diagram viewer; see the
      Frontend section above for what's still missing (mainly automated
      frontend tests)
* [x] Work breakdown (Task Planning) over HTTP -
      `POST`/`GET /requirements-runs/{id}/work-breakdown[/refine|/versions|
      /{version}|/export]`, gated on architecture approval; see "Roadmap"
      → "Task Planning (Work Breakdown Agent)" and "Next Steps" → "16.
...
## Task Planning (Work Breakdown Agent)

Orthogonal to the Milestone 3 checklist above, and to the Milestone 4 and
Milestone 5 items in "Recommended Implementation Order" further down
(this doesn't use that numbering - it builds on an approved architecture
rather than extending the architecture stage itself, and shipped in
parallel):

* [x] `WorkBreakdownArtifact`/`WorkBreakdownExport` domain models
      (`app/domain/work_breakdown.py`), effort-estimated (XS–XL) and
      requirement-/architecture-ID-traceable at the task level
* [x] Microsoft Agent Framework-backed generation + conversational
      refinement (`GenerateWorkBreakdownUseCase`,
      `AgentFrameworkWorkBreakdownAgent`)
* [x] Deterministic, LLM-free CSV export + traceability validation
      (`backend/tools-service`'s `WorkBreakdownExporter` - flags any
      fabricated/uncovered ID rather than silently accepting it)
* [x] MCP tools - `generate_work_breakdown`/`refine_work_breakdown`/
      `export_work_breakdown_csv`, plus a `work-breakdown://schema`
      resource; see "MCP" above
* [x] Web API - `POST`/`GET /requirements-runs/{id}/work-breakdown[/refine|
      /versions|/{version}|/export]`, gated on the session's architecture
      being approved; see "Web API & Authentication" above
* [x] Frontend - the "Task Planning" tab: generate, refine
      conversationally, and download the CSV, appearing once
      architecture is approved; see "Frontend" above
* [x] Manual requirements editing (`PUT /requirements-runs/{id}/requirements`,
      `RequirementsEditor.tsx`) - closes the gap where a diagram-originated
      session (zero functional/non-functional requirements by design) could
      never satisfy Task Planning's traceability precondition; see "Next
      Steps" → "16. Work Breakdown Agent" → "The diagram-originated-session
      gap, and how it's closed"
* [ ] CLI support - the interactive CLI (`app/main.py`) still stops at
      architecture accept/refine; it has no "generate work breakdown"
      step the way the web API and MCP server now do. See "Next Steps" →
      "Work Breakdown Agent" below.
* [ ] Session-state model unification - `SessionRecord` picked up its
...
file's "Next Steps" and "Feature Checklist" sections marking it done - the
same pattern used for every completed Milestone 3 item so far.
...
### Technology-neutral Milestone 2 architecture
...
The next development phase should focus on moving from **Milestone 2 architecture generation** toward a more complete, traceable, and refinement-oriented system design workflow.

## Clean Architecture Migration

**Complete - 8 of 8 slices done.** A separate initiative (not part
of the Milestone 3 feature checklist above) that re-layered the backend as Clean
...
This should remain separate from Milestone 2 so that the high-level architecture does not become unnecessarily implementation-specific.
...
This stage can eventually support cloud-specific architectures, but should remain separate from the technology-neutral Milestone 2 design.
...
## 16. Work Breakdown Agent

**Status: shipped, orthogonal to the numbered roadmap above** - see
"Roadmap" → "Task Planning (Work Breakdown Agent)" for the checklist.
This section is the spec + architecture write-up; the "Current Status"
section at the top of this README and "Capabilities implemented so far"
have the user-facing summary.

**What it does.** Given an approved `RequirementsArtifact` +
`SystemDesignArtifact`, it generates a traceable Feature → Story → Task
implementation plan suitable for import into a tool like Monday.com or
Azure DevOps:

* Every **Feature** groups related **Stories**; every **Story** groups
  concrete **Tasks**.
* Every **Task** carries a short imperative title, a one-to-two sentence
  description, an effort estimate (`XS`/`S`/`M`/`L`/`XL` - see
  `EFFORT_LEGEND` in `app/domain/work_breakdown.py` for what each size
  means), and the `requirement_ids`/`architecture_ids` it traces back
  to. A task with neither is flagged, not silently accepted - see
  "Traceability enforcement" below.
* Ambiguities the agent notices while generating (a requirement with no
  matching architecture element, an architecture element no requirement
  justifies, a genuinely unclear scope boundary) are collected as
  `WorkBreakdownAmbiguity` entries rather than papered over with an
  invented task.

**Why a third/fourth pipeline stage instead of folding it into
architecture generation.** Requirements answer *what the system needs
to do*; architecture answers *how it's structured*; work breakdown
answers *what work someone has to actually do, in what order, at what
size* - a genuinely different question with a different consumer (a
project manager or an engineering lead importing a CSV into a board
tool, not a system designer). Keeping it a separate
`GenerateWorkBreakdownUseCase`/`WorkBreakdownAgentPort` behind its own
stage - gated on architecture being *approved*, not merely generated -
means a rejected or still-being-refined architecture can never produce
a work breakdown against a moving target.

**Traceability enforcement, and why it lives in `backend/tools-service`
rather than the agent itself.** The agent generating the plan is the
same kind of large language model that generates requirements and
architecture, so it's just as capable of inventing a plausible-looking
but nonexistent requirement or architecture ID under refinement
pressure. Rather than trust the agent's own output, the CSV
export/traceability check is a second, independent, deterministic,
LLM-free pass (`backend/tools-service`'s `WorkBreakdownExporter`) that
recomputes the valid ID sets directly from the actual
`RequirementsArtifact`/`SystemDesignArtifact` and set-diffs every
`requirement_ids`/`architecture_ids` reference against them - the same
"don't trust the LLM to grade its own traceability" principle already
used for `ArchitectureValidator`. A task with **zero** IDs at all is
treated as fatal (`WorkBreakdownExportError`, HTTP 422) since it can't
be traced to anything; a fabricated or uncovered ID is downgraded to a
warning in the `WorkBreakdownExport` response instead of blocking
export outright, since a human reviewing the CSV before import is a
reasonable place to catch that rather than refusing to render the file.

**Three surfaces, one use case underneath.** All three call the same
`GenerateWorkBreakdownUseCase`/`WorkBreakdownExporterPort`
(`McpToolsClient.export`) - nothing is duplicated per surface:

* The **external MCP server** - `generate_work_breakdown`/
  `refine_work_breakdown`/`export_work_breakdown_csv` tools, stateless
  like every other MCP tool here (see "MCP" above); a caller passes JSON
  in and gets JSON/CSV back, with no session of its own.
* The **web API** - `POST`/`GET /requirements-runs/{id}/work-breakdown[/refine|
  /versions|/{version}|/export]` (see "Web API & Authentication" above),
  session-aware: it persists each generation as a new immutable version
  (mirroring how requirements/design versioning already works), enforces
  the "architecture must be approved first" and "generate once, then
  refine" rules, and caches the rendered CSV in Blob Storage alongside
  the JSON.
* The **frontend** - the "Task Planning" tab (see "Frontend" above),
  which only appears once a session's architecture is approved: generate
  once, refine conversationally through the same input the architecture
  stage already uses, and download the CSV.

**The diagram-originated-session gap, and how it's closed.**
`GenerateWorkBreakdownUseCase.execute` refuses to run without at least
one functional or non-functional requirement - a work breakdown needs
something to trace to. A session that started from an uploaded diagram
image has zero of either by design (see "Image Input Classification"
below) and, before `PUT /requirements-runs/{id}/requirements` existed,
had no way to acquire any either, since it skips straight past the
requirements stage where `refine` would normally apply. That manual,
non-AI edit route (see "Web API & Authentication" above and
`RequirementsEditor.tsx` in "Frontend") is what unblocks it: it works
from the `"architecture"` stage specifically because `refine` can't, so
a diagram-originated session can pick up real requirements after the
fact and then successfully generate a work breakdown.

**What's still open:**

* **CLI support.** `app/main.py`'s interactive loop still ends at
  architecture accept/refine - there's no "4. Generate work breakdown"
  menu option the way the web API and MCP server now have a work-
  breakdown step. Low-risk to add (the CLI already has a
  `DesignSession`-style wrapper for architecture; a
  `WorkBreakdownSession` following the same shape, calling
  `GenerateWorkBreakdownUseCase` directly, would close this).
* **`SessionRecord`/`DesignSession` duplication.** Adding
  `work_breakdown_version` to `SessionRecord` (web/API side) without a
  CLI equivalent widens the pre-existing gap flagged in "Next
  Development Steps" → "Reconcile the CLI and web session models" above
  - now there are three fields (`requirements_version`,
  `design_version`, `work_breakdown_version`) that would need adding
  twice if the CLI ever catches up, instead of one shared model.
* **No frontend automated tests** for the new "Task Planning" tab -
  consistent with the rest of `frontend/` (see "Frontend" →
  "What's deliberately not covered yet" and "Next Development Steps" →
  "Frontend automated tests" above; this isn't a new gap, just more
  surface area under an existing one).
* **No MCP-level session integration.** The MCP tools remain
  deliberately stateless (see "MCP" above) - an MCP client that wants
  the persisted, versioned breakdown for a specific web-API session has
  to go through the web API's routes instead; that's a deliberate
  design choice, not an oversight, matching how `generate_system_design`
  et al. already work.
...
Milestone 2 is done, and Milestone 3 is now three-sixths complete (refinement,
...
then **ADRs** - see "Roadmap" → "Path to Milestone 3 Completion" above for
...
"Technology-neutral Milestone 2 architecture" below).

Not shown in this diagram: **Task Planning (Work Breakdown Agent)**
shipped in parallel with this Milestone 3 work rather than at a fixed point in
the sequence above, since it depends only on an *approved* architecture
existing - not on which Milestone 3 items are done - and doesn't block or get
blocked by coverage/impact/ADRs/change-history. See "Roadmap" → "Task
Planning (Work Breakdown Agent)" and "Next Steps" → "16. Work Breakdown
Agent" for its own status and remaining gaps (CLI support, mainly).
