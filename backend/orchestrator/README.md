...
backend/orchestrator/    (this directory) — owns all state and every LLM
                          ...
                          Framework-backed requirements/design generation,
                          and the pre-existing external MCP server
                          (app/mcp/server.py) all live here.

backend/mcp-wrapper/      A thin internal MCP gateway. Exposes exactly two
                          tools — generate_architecture_diagram_tool and
                          validate_system_design_tool — that translate an
...
backend/tools-service/    The deterministic, LLM-free logic that used to
                          ...
                          Plain FastAPI REST endpoints — no MCP awareness
                          at all, and no LLM calls.
```

The call path for a diagram render or a validation is:
...
exactly as it did when both were in-process classes. mcp-wrapper's tools
never raise on a tools-service-level failure (a 422 for an invalid
diagram spec or a failed validation is an expected outcome, not a
transport error) — they always return an envelope,
`{"ok": bool, "status_code": int, "body": {...}}`, and `McpToolsClient`
is what turns a `False` `"ok"` into the same
`DiagramGenerationError`/`ArchitectureValidationError` these ports always
raised. See `backend/mcp-wrapper/src/design_tools_wrapper/application
